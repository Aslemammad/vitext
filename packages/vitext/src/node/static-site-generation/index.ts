...
export async function ssrBuild(
  ...
  const ssrOutput = await viteBuild({
    ...
    build: {
      ...
      rollupOptions: {
        ...
        input: path.join(distDir, './client/ssr/serverRender.js'),
    ...
    ssr: {
      ...
      noExternal: ['vitext-theme-basic', 'vitext', 'vitext/client'],
    },
  });
  ...
  const _clientResult = await viteBuild({
    ...
    build: {
      cssCodeSplit: false,
      rollupOptions: {
        input: path.join(distDir, './client/ssr/clientRender.js'),
        preserveEntrySignatures: 'allow-extension',
      },
      assetsDir: '_assets',
      outDir: clientOutDir,
